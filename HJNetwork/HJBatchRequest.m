...
@property (nonatomic, strong, readwrite) NSMutableArray<HJRequest *> *failedRequestArray;
@end


@implementation HJBatchRequest

- (instancetype)initWithRequestArray:(NSArray<HJRequest *> *)requestArray {
    self = [super init];
    if (self) {
        _requestArray = [requestArray copy];
        _failedRequestArray = @[].mutableCopy;
        _finishedCount = 0;
        for (HJRequest * req in _requestArray) {
            if (![req isKindOfClass:[HJRequest class]]) {
                HJLog(@"Error, request item must be HJRequest instance.");
                return nil;
            }
        }
    }
...
- (BOOL)isDataFromCache {
    BOOL result = YES;
    for (HJRequest *request in _requestArray) {
...
- (void)requestFinished:(HJRequest *)request {
    _finishedCount++;
    if (_finishedCount == _requestArray.count) {
        if (_failedRequestArray.count) {
            [self _handleFailedRequest];
        } else {
            [self toggleAccessoriesWillStopCallBack];
...
- (void)requestFailed:(HJRequest *)request {
    _finishedCount++;

 ...
 *  \returns An updated value of bytesInserted or kCFNotFound if the URL must be reparsed.
 */
static CFIndex LowercaseHost(NSURL *url, NSMutableData *urlData, CFIndex bytesInserted) { // The host should be lower case; if it's not, make it so.
    CFRange     range;
    uint8_t *   urlDataBytes;
    CFIndex     i;
...
extern NSMutableURLRequest * HJCanonicalRequestForRequest(NSURLRequest *request) {
    ...
    // First up check that we're dealing with HTTP or HTTPS.  If not, do nothing (why were we we even called?).
    ...
    if (![scheme isEqual:@"http" ] && ![scheme isEqual:@"https"]) {
        ...
        CFIndex bytesInserted = kCFNotFound;
        NSMutableData *urlData = nil;
        size_t stepIndex;
        size_t stepCount = sizeof(kStepFunctions) / sizeof(*kStepFunctions);
        for (stepIndex = 0; stepIndex < stepCount; stepIndex++) {
            // If we don't have valid URL data, create it from the URL.
            assert(requestURL != nil);
            if (bytesInserted == kCFNotFound) {
                NSData *urlDataImmutable = CFBridgingRelease( CFURLCreateData(NULL, (CFURLRef) requestURL, kCFStringEncodingUTF8, true) );

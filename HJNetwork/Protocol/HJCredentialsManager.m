//
//  HJCredentialsManager.m
//  HJNetwork
//
//  Created by navy on 2022/5/30.
//

#import "HJCredentialsManager.h"
@import Security;

@interface HJCredentialsManager ()
...
@implementation HJCredentialsManager
...
- (void)addTrustedAnchor:(SecCertificateRef)newAnchor {
    ...
    @synchronized (self) {
        ...
        if (!found) {
            NSIndexSet *indexSet = [NSIndexSet indexSetWithIndex:[self->_mutableTrustedAnchors count]];
            assert(indexSet != nil);

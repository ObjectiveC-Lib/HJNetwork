...
static NSString *const kHJSessionIncompleteDownloadFolderName = @"HJSessionIncomplete";

typedef void (^AFURLSessionProgressiveOperationProgressBlock)(HJDownloadSessionTask *task, NSInteger bytes,
                                                              long long totalBytes,
                                                              long long totalBytesExpected,
                                                              long long totalBytesReadForFile,
                                                              long long totalBytesExpectedToReadForFile);

@interface HJDownloadSessionTask () {
}
...
@implementation HJDownloadSessionTask
...
- (instancetype)initWithTask:(NSURLSessionDataTask *)task
    ...
    if (self) {
        ...
        if (!self.outputStream) {
            NSLog(@"Failed to create %@ outputStream",NSStringFromClass(self.class));
        }
    }
    return self;
}
...
+ (NSString *)cacheFolder {
    ...
    if (!cacheFolder) {
        ...
        cacheFolder = [cacheDir stringByAppendingPathComponent:kHJSessionIncompleteDownloadFolderName];
    }
...
@interface HJDownloadSessionManager ()
@property (nonatomic, strong) HJNetworkConfig *config;
@end

@implementation HJDownloadSessionManager {
}
...
- (HJDownloadSessionTask *)downloadTaskBreakPointsWithRequest:(NSURLRequest *)request
    ...
    if (self.config.useDNS) {
        ...
        if (node) {
            if (node.url != nil && [node.url length] > 0) {
                mutableRequest.URL = [NSURL URLWithString:node.url];
            }
            if (node.host != nil && [node.host length] > 0) {
                [mutableRequest setValue:node.host forHTTPHeaderField:@"host"];
            }
        }
    }
    ...
    HJDownloadSessionTask *afTask = [[HJDownloadSessionTask alloc] initWithTask:task
...
didReceiveResponse:(NSURLResponse *)response
 completionHandler:(void (^)(NSURLSessionResponseDisposition disposition))completionHandler {
    HJDownloadSessionTask *downloadTask = [self getAFDownloadTaskForDataTask:dataTask];
    if (downloadTask) {
                ...
                if (file) { // maybe nil init error
                    @try {
                        ...
                        error = [NSError errorWithDomain:exception.reason
                                                    code:HJDownloadSessionTaskErrorNoSpace
                    ...
                    error = [NSError errorWithDomain:[NSString stringWithFormat:@"%@",NSStringFromClass([self class])] code:HJDownloadSessionTaskErrorNoSpace userInfo:@{NSLocalizedFailureReasonErrorKey:@"no space"}];
                    downloadTask.fileError = error;
                    downloadTask.responseObject = nil;
                }
            }
...
- (void)URLSession:(NSURLSession *)session task:(NSURLSessionTask *)task didCompleteWithError:(NSError *)error {
    HJDownloadSessionTask *downloadTask = [self getAFDownloadTaskForDataTask:task];
    if (downloadTask) {
        [downloadTask.outputStream close];
...
- (HJDownloadSessionTask *)getAFDownloadTaskForDataTask:(NSURLSessionTask *)task {
    HJDownloadSessionTask *downloadTask = nil;
    for (HJDownloadSessionTask *aDownloadTask in self.afDownloadTasks) {

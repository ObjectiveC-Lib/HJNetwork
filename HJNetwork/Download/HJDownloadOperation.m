...
typedef void (^AFURLConnectionProgressiveOperationProgressBlock)(HJDownloadOperation *operation,
                                                                 NSInteger bytes,
...
- (void)setProgressiveDownloadProgressBlock:(void (^)(HJDownloadOperation *operation,
                                                      NSInteger bytesRead,
...
- (void)connection:(NSURLConnection *)connection didReceiveData:(NSData *)data {
    ...
    if (![self.responseSerializer validateResponse:self.response data:data ?: [NSData data] error:NULL]) return;
    ...
    if (self.progressiveDownloadProgress) {
        dispatch_async(self.progressiveDownloadCallbackQueue ?: dispatch_get_main_queue(), ^{
            self.progressiveDownloadProgress(self,(NSInteger)[data length],
                                             self.totalBytesRead,
...
+ (NSString *)cacheFolder {
    ...
    if (!cacheFolder) {
        ...
        cacheFolder = [cacheDir stringByAppendingPathComponent:kHJOperationIncompleteDownloadFolderName];
    }

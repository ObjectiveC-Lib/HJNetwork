...
+ (void)setCertificatePublicKeys:(NSArray *)keys {
    ...
    @synchronized([HJCredentialChallenge class]) {
        if (_globalExtendCAs) {
            [_globalExtendCAs addObjectsFromArray:keys];
        } else {
            _globalExtendCAs = [NSMutableSet setWithArray:keys];
        }
    }
}
...
+ (void)setProxyEnable:(BOOL)enable {
    [[NSUserDefaults standardUserDefaults] setBool:enable forKey:@"HJEnableProxy"];
}

...
- (instancetype)init {
    ...
    if (self) {
        ...
        [_manager setSessionDidReceiveAuthenticationChallengeBlock:_config.sessionDidReceiveAuthenticationChallenge];
        [_manager setAuthenticationChallengeHandler:_config.authenticationChallengeHandler];
...
- (void)requestDidFailWithRequest:(HJBaseRequest *)request error:(NSError *)error {
    request.error = error;
    HJLog(@"Request %@ failed, status code = %ld, error = %@", NSStringFromClass([request class]), (long)request.responseStatusCode, error.localizedDescription);

...
@interface AFDownloadSessionTask () {
}
...
@implementation AFDownloadSessionTask
...
- (void)setProgressiveDownloadProgressBlock:(void (^)(AFDownloadSessionTask *task,
                                                      NSInteger bytesRead,
...
- (AFDownloadSessionTask *)downloadTaskBreakPointsWithRequest:(NSURLRequest *)request
    ...
    if (self.config.useDNS) {
        ...
        if (node) {
            if (node.realUrl != nil && [node.realUrl length] > 0) {
                mutableRequest.URL = [NSURL URLWithString:node.realUrl];
            }
            if (node.realHost != nil && [node.realHost length] > 0) {
                [mutableRequest setValue:node.realHost forHTTPHeaderField:@"host"];
            }
        }
    }
    ...
    AFDownloadSessionTask *afTask = [[AFDownloadSessionTask alloc] initWithTask:task
...
didReceiveResponse:(NSURLResponse *)response
 completionHandler:(void (^)(NSURLSessionResponseDisposition disposition))completionHandler {
    AFDownloadSessionTask *downloadTask = [self getAFDownloadTaskForDataTask:dataTask];
    if (downloadTask) {
                ...
                if (file) { // maybe nil init error
                    @try {
                        ...
                        error = [NSError errorWithDomain:exception.reason
                                                    code:AFDownloadSessionTaskErrorNoSpace
                    ...
                    error = [NSError errorWithDomain:[NSString stringWithFormat:@"%@",NSStringFromClass([self class])] code:AFDownloadSessionTaskErrorNoSpace userInfo:@{NSLocalizedFailureReasonErrorKey:@"no space"}];
                    downloadTask.fileError = error;
                    downloadTask.responseObject = nil;
                }
            }
...
- (void)URLSession:(NSURLSession *)session task:(NSURLSessionTask *)task didCompleteWithError:(NSError *)error {
    AFDownloadSessionTask *downloadTask = [self getAFDownloadTaskForDataTask:task];
    if (downloadTask) {
        [downloadTask.outputStream close];
...
- (AFDownloadSessionTask *)getAFDownloadTaskForDataTask:(NSURLSessionTask *)task {
    AFDownloadSessionTask *downloadTask = nil;
    for (AFDownloadSessionTask *aDownloadTask in self.afDownloadTasks) {
